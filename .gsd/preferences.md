...
post_unit_hooks:
  ...
  - name: sync-milestone-docs
    ...
    prompt: |
      ...
      如果不存在，创建完整文件，内容如下（包含 XML 标记区域）：

      # Agent Guide

      <!-- GSD:AUTO:START — 以下内容由 GSD 自动生成，请勿手动编辑 -->
      ...
      如果已存在，只替换 <!-- GSD:AUTO:START --> 和 <!-- GSD:AUTO:END --> 之间的内容（含这两行标记本身重新写入）。
      标记之外的所有内容（用户自定义的偏好、习惯、规则等）保持不变，不要删除或修改。
